...
def isPathThere(path):
    ...
    if not os.path.exists(path):
        print >> sys.stderr, 'Path (%s) could not be found on %s' % (path,
                                                                     host)
        sys.exit(-1)
...
# ------------ get machine list
machines = (os.path.exists(machineListFile)
            and [x[:-1] for x in open(machineListFile).readlines()]
            or machineListFile.split(','))
...
def get_synclist_from_cmdline(tmp):
    ...
    list = (os.path.exists(synclist)
            and [x[:-1] for x in open(synclist).readlines()]
            or synclist.split(','))
...
def get_synclist_from_rsync(tmp):
    ...
    for machine in machines:
        ...
        if (not cmdToTarget.isMachineOK(reshell,
                                        machine)):  # if not go to next machine
            continue
...
(synclist and get_synclist_from_cmdline
 or get_synclist_from_rsync)(tmp_synclist())
    ...
    return ' '.join(
        filter(
            ...
            [
                ...
                '</dev/null 1>/dev/null 2>%s &' % catcher_err_log
            ]))
    ### workaround ssh problem
...
def invoke_catcher(ms, count, bads):
    ...
    if (not status):
        if verbose >= 1:
            print >> sys.stderr, "***remote shell exec failed for %s" % machine
        bads.append(machine)
        return invoke_catcher(ms, count, bads)
...
def gen_caster_cmd():
    ...
    return ' '.join(
        filter(lambda x: x, [
            ...
            (mcast_addr and '-A ' + mcast_addr or ''),
            (port > 0 and '-P %d' % port or ''),
            (ttl > 1 and '-T %d' % ttl or ''), (loop and '-L' or ''),
            (mcast_if and '-I %s' % mcast_if or ''),
